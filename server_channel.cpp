...
void Server::joinCommand(int userFd, std::string channelName, std::string key)
{
    ...
    else
    {
        ...
        if (channel->isInviteOnly() && !channel->isInvited(userFd)) {
            ...
            sendToClient(userFd, ERR_INVITEONLYCHAN, channelName + " :Cannot join channel (+i)"); // Taha trying to fix lime chat
			
            return;
        }
        if (!channel->canJoin(key)){
			// sendToClient(userFd, ERR_BADCHANNELKEY, channelName + ": wrong key");
            sendToClient(userFd, ERR_BADCHANNELKEY, channelName + " :Cannot join channel (+k)"); // Taha trying to fix lime chat
            return;
        }
        if (channel->isFull()) {
			// sendToClient(userFd, ERR_CHANNELISFULL, channelName + ": channel is full");
            sendToClient(userFd, ERR_CHANNELISFULL, channelName + " :Cannot join channel (+l)"); // Taha trying to fix lime chat
            return;
        }
        channel->addUser(userFd, this->_clients[userFd]);
    }
    ...
    std::string joinMsg = ":" + _clients[userFd]->getPrefix() + " JOIN " + channelName; // Taha trying to fix lime chat
    ...
    if (!(channel->getTopic().empty()))
	{
        ...
        sendToClient(userFd, RPL_TOPIC, channelName + " :" + channel->getTopic()); // Taha trying to fix lime chat
	}
    else
	{
		// sendToClient(userFd, RPL_NOTOPIC , " " + channelName);
        sendToClient(userFd, RPL_NOTOPIC, channelName + " :No topic is set"); // Taha trying to fix lime chat
	}
    ...
    sendToClient(userFd, RPL_NAMREPLY, "= " + channelName + " :" + userList); // Taha trying to fix lime chat	
    sendToClient(userFd, RPL_ENDOFNAMES, channelName + " :End of /NAMES list"); // Taha trying to fix lime chat
}
